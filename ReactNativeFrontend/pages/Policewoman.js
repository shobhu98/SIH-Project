...
export default class App extends React.Component {
  ...
  checkBot(messages){
    if(this.state.append===0){
      ...
      messages[0].text="पीएच "+messages[0].text;
      this.setState({append:this.state.append+1})
  ...
  onSend(messages) {
    ...
    this.setState(previousState => ({
      ...
      response.json().then((result) => {
        if (response.status === 200) {
          console.log(result.fulfillmentMessages[0].text.text[0]);
          if(result.fulfillmentMessages[0].text.text[0].localeCompare("अब मुझे आपका फोन नंबर बताइए |")===0){
            console.log("Changing here");
            this.setState({append:this.state.append+1});
          }
          var js = {
          ...
          Speech.speak(result.fulfillmentMessages[0].text.text[0],{language:"hi-IN", pitch:0.5, rate:0.8, 
            onStart:start ,
            onDone: end
          });
